...
class MultiAgentTripPlanner:
    ...
    def plan_trip(self, request: TripRequest) -> TripPlan:
        ...
        try:
            ...
            # 步骤1-3: 并行调用三个独立Agent (景点搜索 / 天气查询 / 酒店搜索)
    ...
    def _build_attraction_query(self, request: TripRequest) -> str:
        ...
        if request.preferences:
            ...
            # 为每种偏好分别生成工具调用指令
            tool_calls = "\n".join([
                f"[TOOL_CALL:amap_maps_text_search:keywords={pref},city={request.city}]"
                for pref in request.preferences
            ])
        else:
            keywords = "景点"
            tool_calls = f"[TOOL_CALL:amap_maps_text_search:keywords=景点,city={request.city}]"

        query = f"请使用amap_maps_text_search工具搜索{request.city}的以下类型景点: {keywords}\n{tool_calls}"
        return query
...
"""
    ...
    def _create_fallback_plan(self, request: TripRequest) -> TripPlan:
        ...
        # 尝试获取城市的真实坐标(通过MCP工具)
        city_location = self._try_get_city_location(request.city)
            ...
            day_plan = DayPlan(
                ...
                attractions=[
                    Attraction(
                        ...
                        location=Location(
                            longitude=city_location["longitude"] + i * 0.01 + j * 0.005,
                            latitude=city_location["latitude"] + i * 0.01 + j * 0.005
                        ),
                        visit_duration=120,
    ...
    def _try_get_city_location(self, city: str) -> dict:
        """尝试通过MCP工具获取城市默认坐标，失败则返回北京坐标"""
        try:
            query = f"[TOOL_CALL:amap_maps_text_search:keywords={city},city={city}]"
            response = self.attraction_agent.run(query)
            print(f"备用坐标查询结果: {response[:200]}...")
            # 尝试从响应中提取坐标，失败则使用默认坐标
            import re
            coords = re.findall(r'"location"\s*:\s*"([\d.]+),([\d.]+)"', response)
            if coords:
                return {"longitude": float(coords[0][0]), "latitude": float(coords[0][1])}
        except Exception as e:
            print(f"获取城市坐标失败: {e}")
        # 默认城市坐标映射
        city_coords = {
        ...
        return city_coords.get(city, {"longitude": 116.4074, "latitude": 39.9042})
