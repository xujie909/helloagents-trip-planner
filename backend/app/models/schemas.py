...
class ParseRequest(BaseModel):
    """对话解析请求"""
    message: str = Field(..., description="用户输入的自然语言")
    current_fields: CurrentFields = Field(default_factory=CurrentFields, description="当前已提取的字段")
...
class ExtractedFields(BaseModel):
    ...
    extra_asked: bool = Field(default=False, description="用户是否表示没有其他要求了")

class ParseResponse(BaseModel):
    """对话解析响应"""
    success: bool = Field(default=True, description="是否成功")
    extracted: ExtractedFields = Field(default_factory=ExtractedFields, description="新提取的字段")
    bot_reply: Optional[str] = Field(default=None, description="AI 回复文本")
    is_complete: bool = Field(default=False, description="信息是否收集完整")

# ============ 视频生成模型 ============

class VideoGenerateRequest(BaseModel):
    """视频生成请求"""
    scenic_name: str = Field(..., description="景区名称", example="故宫")
    trip_id: Optional[str] = Field(default=None, description="关联旅程ID")
    trip_title: Optional[str] = Field(default=None, description="关联旅程标题")
    trip_city: Optional[str] = Field(default=None, description="关联旅程城市")
    source_attraction: Optional[str] = Field(default=None, description="来源景点名称")


class VideoTaskStatus(BaseModel):
    """视频生成任务状态"""
    task_id: str = Field(..., description="任务ID")
    status: str = Field(..., description="任务状态: processing | done | error")
    progress: int = Field(default=0, description="进度 0-100")
    message: str = Field(default="", description="状态消息")
    video_url: Optional[str] = Field(default=None, description="视频下载URL（完成后）")
    trip_id: Optional[str] = Field(default=None, description="关联旅程ID")
    trip_title: Optional[str] = Field(default=None, description="关联旅程标题")
    trip_city: Optional[str] = Field(default=None, description="关联旅程城市")
    source_attraction: Optional[str] = Field(default=None, description="来源景点名称")
...
class VideoHistoryItem(BaseModel):
    """视频历史记录项"""
    task_id: str = Field(..., description="任务ID")
    scenic_name: str = Field(default="", description="景区名称")
    status: str = Field(..., description="任务状态")
    progress: int = Field(default=0, description="任务进度")
    message: str = Field(default="", description="状态消息")
    created_at: str = Field(default="", description="创建时间")
    updated_at: str = Field(default="", description="更新时间")
    video_url: Optional[str] = Field(default=None, description="可下载视频地址")
    file_exists: bool = Field(default=False, description="视频文件是否仍然存在")
    trip_id: Optional[str] = Field(default=None, description="关联旅程ID")
    trip_title: Optional[str] = Field(default=None, description="关联旅程标题")
    trip_city: Optional[str] = Field(default=None, description="关联旅程城市")
    source_attraction: Optional[str] = Field(default=None, description="来源景点名称")
...
class VideoHistoryResponse(BaseModel):
    ...
    success: bool = Field(default=True, description="是否成功")
    message: str = Field(default="", description="消息")
    data: List[VideoHistoryItem] = Field(default_factory=list, description="视频历史列表")
...
class FAQMatchResult(BaseModel):
    ...
    item: Optional[FAQItem] = Field(default=None, description="匹配到的 FAQ")
